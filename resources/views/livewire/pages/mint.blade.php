<div>
    ...
    <div class="mt-16 md:flex items-start">
      <div class="md:w-1/2 pr-5 md:sticky top-10">
        @if ($smart_contract->artwork_path)
          ...
          @else
            <video class="w-full" controls loop playsinline poster="{{ $smart_contract->getPreviewArtworkUrl() }}" onmouseover="this.play()" onmouseout="this.pause();this.currentTime=0;">
      ...
      <div class="md:w-1/2 pt-10 md:pt-0 md:pl-5">
        ...
        <div>
          ...
          <div class="flex gap-10" x-data="{ openModal: false }" >
          @if (!$is_deployed)
  ...
  <script>
    ...
    async function switchTheNetwork() {
    ...
    async function mint() {
      if (@this.network_public_id == getNetwork().chain.id &&
        formatAddress(getAccount().address) != '' &&
        @this.smart_contract_address != '' &&
        @this.smart_contract_price != '') {
